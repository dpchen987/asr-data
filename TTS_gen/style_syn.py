...
#     for utt_id, sentence in sentences:
    t2t_pth = str(output_dir / "tts_trans.txt")
    s2t_pth = str(output_dir / "tts_wav_scp.txt")
    t2d_pth = str(output_dir / "tts_dev_trans.txt")
    s2d_pth = str(output_dir / "tts_dev_wav_scp.txt")
    t2ts_pth = str(output_dir / "tts_test_trans.txt")
    s2ts_pth = str(output_dir / "tts_test_wav_scp.txt")
    with open(args.text, 'rt', encoding="utf8", errors="ignore") as f, open(t2t_pth, "w") as t2t_f, open(s2t_pth, "w") as s2t_f, open(t2d_pth, "w") as t2d_f, open(s2d_pth, "w") as s2d_f, open(t2ts_pth, "w") as t2ts_f, open(s2ts_pth, "w") as s2ts_f, paddle.no_grad():
        stop_num = 0
        for line in f:
            # stop_num += 1
